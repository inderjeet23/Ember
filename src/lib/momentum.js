export const STATE_MIN = { fresh: 70, warm: 40, cooling: 15, cold: 0 };
export function stateFromScore(score) {
    if (score >= STATE_MIN.fresh)
        return 'fresh';
    if (score >= STATE_MIN.warm)
        return 'warm';
    if (score >= STATE_MIN.cooling)
        return 'cooling';
    return 'cold';
}
function clamp(n, min = 0, max = 100) { return Math.max(min, Math.min(max, n)); }
function decayMultiplier(type) {
    if (type === 'writing')
        return 0.9;
    if (type === 'dev')
        return 1.1;
...
export function applyGain(p, kind) {
    ...
    const win = { id: Math.random().toString(36).slice(2), projectId: p.id, type: (kind === 'railOverflow' ? 'taskComplete' : kind), value: delta, createdAt: Date.now() };
    p.wins.unshift(win);
    return win;
}
export function dailyRecompute(p, now = Date.now()) {
